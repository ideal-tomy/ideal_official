...
- [x] 自由入力 → API → ask / draft（`OPENAI_API_KEY` 設定時）
- [x] 最終結果が ConciergeResult カードに載る（`mergeAiDraftIntoResult`）
- [x] API未設定・障害時は「選択内容だけで結果へ」で Contact まで到達

### 実装結果（2026-07-10）

| ファイル | 役割 |
|----------|------|
| `app/api/concierge/chat/route.ts` | API Route（レート制限・キー確認） |
| `lib/concierge/ai/*` | prompts / OpenAI client / parse / schema |
| `components/concierge/ConciergeAiStep.tsx` | 自由入力・追加質問UI |
| `merge-ai-draft.ts` | ドラフトを結果カードへマージ |

環境変数: [`.env.example`](../.env.example) の `OPENAI_API_KEY`（任意で `OPENAI_MODEL` / `OPENAI_BASE_URL`）。
...
## Phase C5 — 磨き込み・営業デモ化 ✅ 完了 (2026-07-10)

### やること

- [x] 主要デモ7本・公開Cases・主要サービス3つのオープニング文面を揃える
- [x] TOP / Gallery CTA コピーを「自社でも使えるか相談」に寄せる
- [x] 営業用シナリオをドキュメント化 → [`AI_CONCIERGE_SALES_SCENARIO.md`](./AI_CONCIERGE_SALES_SCENARIO.md)
- [x] モバイル: `dvh` / safe-area / 結果カード高さ調整

### 実装結果

| 領域 | 内容 |
|------|------|
| Openings | `DEMO_OPENING_COPY` / `CASE_OPENING_PROMPTS` / Insights タイトル起点 |
| CTA | Gallery / デモ詳細 / Cases / TOP CTA / DemoEntry / ServiceAiCta |
| モバイル | FAB safe-area、PremiumDialog `dvh`、Done カード `42dvh` |
| 営業 | `docs/AI_CONCIERGE_SALES_SCENARIO.md` |

### 任意（未着手）

- SERVICES / LAB に「AIコンシェルジュ型サイト」紹介セクションを短く追加
...
1. `.env.local` に `OPENAI_API_KEY` を入れて AI整理を目視確認
2. [`AI_CONCIERGE_SALES_SCENARIO.md`](./AI_CONCIERGE_SALES_SCENARIO.md) で営業デモ通し
3. 価格表（`pricing-rules.ts`）のレンジを実案件に合わせて調整

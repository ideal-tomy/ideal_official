...
export function FolderTree({ folders, results, isComplete }: FolderTreeProps) {
  ...
  return (
    <div className="rounded-lg border border-[#D9DDE3] bg-white p-4 space-y-4">
      <p className="text-xs font-medium text-[var(--site-fg-muted)] uppercase tracking-wider">
        整理結果
      </p>
          ...
          {folders.map((folder) => {
            ...
            return (
              <li key={folder} className="text-sm text-gray-600">
                <span className="text-[var(--site-fg-muted)]">├─</span> {folder}/
                <span className="text-xs text-[var(--site-fg-muted)] ml-1">({count})</span>
              </li>
            )
          })}
        </ul>
      ...
      <div className="border-t border-[#D9DDE3] pt-3">
        <p className="text-xs font-medium text-[var(--site-fg-muted)] mb-2">リネーム一覧</p>
        <ul className="space-y-2">
          {results.map((result) => (
            <li key={result.id} className="text-xs">
              <p className="font-mono text-[var(--site-fg-muted)] line-through truncate">
                {result.originalName}
              </p>
